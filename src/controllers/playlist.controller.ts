...
export const playlistController = {
  async list(req: Request, res: Response): Promise<void> {
    ...
    const result = await contentService.listByType('playlist', {
  ...
  async getById(req: Request, res: Response): Promise<void> {
    const playlist = await contentService.getByIdAndType(Number(req.params.id), 'playlist')
    if (!playlist) {
  ...
  async create(req: Request, res: Response): Promise<void> {
    ...
    const playlist = await contentService.create({
  ...
  async update(req: Request, res: Response): Promise<void> {
    ...
    const existing = await contentService.getByIdAndType(id, 'playlist')
    if (!existing) {
      res.status(404).json({ success: false, message: 'Not found' })
      return
    }
    ...
    const playlist = await contentService.update(id, {
